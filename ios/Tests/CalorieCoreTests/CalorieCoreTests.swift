...
final class CalorieCoreTests: XCTestCase {
    ...
    func testLegacyImportMergesUniqueCloudAndIPhoneRecordsIdempotently() throws {
        ...
        XCTAssertEqual(merged.profile.name, "Sarthak", "A fresh import must restore the legacy profile")

        local.profile.name = "Local owner"
        let localWins = CloudJournalMapper.mergeLegacyImport(into: local, cloud: cloud)
        XCTAssertEqual(localWins.profile.name, "Local owner", "An existing local profile remains authoritative")
        XCTAssertEqual(
