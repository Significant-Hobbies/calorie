...
public enum CloudJournalMapper {
    ...
    public static func mergeLegacyImport(
        ...
        var document = local
        // A fresh install has no local profile yet; carry the legacy profile
        // forward. If this device already has a configured profile, retain it
        // as the established local-wins conflict rule from the old merge flow.
        if local.profile == Profile() {
            document.profile = cloud.document.profile
        }
        ...
        // Legacy export v2 has no cycle context, appearance, or daily-note
        // fields. Those values are intentionally retained from this device;
        // the mirror format carries them separately once sync is established.
        return document
    }
